...
module.exports = {
    config: {
        ...
        role: 0,
        shortDescription: "",
        longDescription: "شات مزيف",
        category: "متعة",
        guide: "{p} منشن | 1{النص} | 2{النص} أو {P}شات_مزيف منشن | النص"
    },

    onStart: async function ({ api, event, args }) {
        const mention = Object.keys(event.mentions);
        if (mention.length === 0) return api.sendMessage(" ⚠️ | عذرا لم تقم بكتابته بطريقة صحيحة مثال : @منشن | النص أو @منشن النص الأول | النص الثاني", event.threadID, event.messageID);
        ...
        if (!mentionedUserProfilePic) {
            return api.sendMessage(" ❌ | فشل إرسال محتوى الرسالة", event.threadID, event.messageID);
        }
